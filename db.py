...
st.title("Simple User Management App by Josh Andal")
...
if choice == "Add User":
    ...
    if not df.empty:
        ...
        if user_ids:
            user_id_to_delete = st.selectbox("Select ID to delete", user_ids)
        else:
        ...
        if st.button("Delete User") and user_id_to_delete:
            ...
            st.success(f"User with ID **{user_id_to_delete}** deleted successfully! 🗑️")
            st.rerun()

    else:
        st.info("The user table is currently empty.")